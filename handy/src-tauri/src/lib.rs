...
use managers::session::{SessionManager, Source};
...
pub fn run(cli_args: CliArgs) {
    ...
    let specta_builder = Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            ...
            commands::models::is_diarization_available,
            ...
            commands::session::set_playback_active,
    ...
    // `handy --toggle-system-session`) is a second instance that runs this same startup
    // path before forwarding its flag — launched from a read-only CWD the relative
    // `../src/bindings.ts` write fails, and a panic here would swallow the flag. Log and
    // continue; the primary `bun tauri dev` (run from src-tauri) still writes bindings.
    #[cfg(debug_assertions)]
    if let Err(e) = specta_builder.export(
        Typescript::default().bigint(BigIntExportBehavior::Number),
        "../src/bindings.ts",
    ) {
        eprintln!("Skipped exporting TypeScript bindings (continuing): {e}");
    }
    ...
    builder
        .plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            ...
            } else if args.iter().any(|a| a == "--toggle-session") {
                match app.state::<Arc<SessionManager>>().toggle(Source::Mic) {
                    Ok(active) => log::info!("Long-form session toggled; active = {active}"),
