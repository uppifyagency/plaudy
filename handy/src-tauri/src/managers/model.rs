...
/// One ONNX model the local speaker diarizer needs. Downloaded as a **bare `.onnx`**
/// (no archive) into `<models_dir>/diarization/` under the fixed name the engine reads
/// (`DiarizationManager::{SEG_FILE,EMB_FILE}`). Bare files mean the existing file-based
/// download/verify path is reused verbatim — no `.tar.bz2`/bzip2 branch, no entry in the
/// ASR `available_models` catalog (which would risk being auto-selected as the ASR model),
/// and no `EngineType::Diarizer` (which would force churn in transcription.rs match arms).
struct DiarizationModelDl {
    /// Destination filename inside the `diarization/` subdir (the engine's expected name).
    filename: &'static str,
    url: &'static str,
    sha256: &'static str,
    /// Expected byte size — fallback when the CDN omits Content-Length, and a sanity bound.
    size: u64,
}

/// Segmentation = pyannote-segmentation-3.0 (HuggingFace mirror, bare model.onnx);
/// embedding = NeMo TitaNet-small (k2-fsa speaker-recognition-models GitHub release).
/// Both pinned by SHA256 computed from the actual downloaded files (2026-06-22).
const DIARIZATION_MODELS: &[DiarizationModelDl] = &[
    DiarizationModelDl {
        filename: DiarizationManager::SEG_FILE,
        url: "https://huggingface.co/csukuangfj/sherpa-onnx-pyannote-segmentation-3-0/resolve/main/model.onnx",
        sha256: "220ad67ca923bef2fa91f2390c786097bf305bceb5e261d4af67b38e938e1079",
        size: 5_992_913,
    },
    DiarizationModelDl {
        filename: DiarizationManager::EMB_FILE,
...
impl ModelManager {
    pub fn new(app_handle: &AppHandle) -> Result<Self> {
        ...
        // Install bundled diarization models (if shipped in resources/) on first run
        manager.migrate_bundled_diarization_models()?;
    ...
    /// Install the bundled speaker-diarization models from the app's `resources/` into the
    /// app-data `diarization/` dir on first run, so a fresh checkout has "who said what"
    /// ready offline — no download, no manual copy. Mirrors `migrate_bundled_models`, and is
    /// a no-op once the user-dir copies exist or when the models aren't bundled (then the
    /// `download_diarization_models` command is the fallback). Filenames come from the shared
    /// `DiarizationManager` consts so the bundle path and the engine's read path can't drift.
    fn migrate_bundled_diarization_models(&self) -> Result<()> {
        let dest_dir = self.models_dir.join(DiarizationManager::SUBDIR);
        for filename in [DiarizationManager::SEG_FILE, DiarizationManager::EMB_FILE] {
            let user_path = dest_dir.join(filename);
            if user_path.exists() {
                continue;
            }
    ...
    /// True when both diarization models are present on disk (diarization can run).
    /// Reuses the engine's own availability check so the path convention stays in one place.
    pub fn diarization_models_available(&self) -> bool {
        DiarizationManager::new(&self.models_dir).is_available()
    }

    /// Download the speaker-diarization models into `<models_dir>/diarization/`.
    /// Idempotent: files already present are skipped, so this is safe to call repeatedly
    /// (e.g. from a UI "Enable diarization" button). Each file is streamed to a `.partial`,
    /// size- and SHA256-verified, then atomically renamed into place.
    ///
    /// ponytail: no resume/range/cancel here (unlike `download_model`) — both files are
    /// <40 MB, so a failed download just retries from scratch. Ceiling: a flaky connection
    /// restarts the whole file. Upgrade path: route these through `download_model`'s resume
    /// loop if that ever bites. Progress is emitted on the existing `model-download-progress`
    /// channel with `model_id = "diarization"` so any UI progress bar works unchanged.
    pub async fn download_diarization_models(&self) -> Result<()> {
        let dir = self.models_dir.join(DiarizationManager::SUBDIR);
        ...
        let _ = self.app_handle.emit("diarization-models-ready", ());
        info!("Diarization models ready in {:?}", dir);
        Ok(())
    }
    ...
    async fn download_diarization_file(
        ...
        // Size guard: a truncated download must not be silently accepted.
        ...
        // SHA256 in a blocking thread (reuses the catalog verifier; deletes the partial on
        // mismatch so the next attempt starts clean).
        let verify_path = partial.clone();
        let verify_sha = model.sha256.to_string();
        let verify_id = model.filename.to_string();
        tokio::task::spawn_blocking(move || {
            Self::verify_sha256(&verify_path, Some(&verify_sha), &verify_id)
        })
        .await
        .map_err(|e| anyhow::anyhow!("SHA256 task panicked: {}", e))??;

        fs::rename(&partial, dest)?;
        info!("Diarization model {} ready", model.filename);
        Ok(())
    }
...
mod tests {
    ...
    // ── Diarization download table ────────────────────────────────────────────

    #[test]
    fn diarization_table_matches_engine_filenames_and_is_well_formed() {
        // The downloader must write exactly the two files the engine reads, with no
