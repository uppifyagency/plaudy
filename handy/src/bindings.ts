 ...
 */
async isDiarizationAvailable() : Promise<Result<boolean, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("is_diarization_available") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async getSessionSegments(historyId: number) : Promise<Result<PersistedSegment[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_session_segments", { historyId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type PersistedSegment = { start_ms: number; end_ms: number; speaker_label: string | null; text: string }
