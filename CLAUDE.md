...
- **Menu-bar "ear" + seamless auto-capture R&D (2026-06-23, this session):** the tray icon now becomes an **ear** (`TrayIconState::Listening`, `resources/tray_listening.png` from the SF Symbol) whenever a session is recording — the honest "I'm listening" signal; dictation keeps the dot. **Auto-capture engine** built (`managers/auto_capture.rs`: unit-tested debounce brain + supervisor with probation/discard/cooldown; `audio_toolkit/audio/output_sensor.rs`; `SessionManager::cancel`/`system_audio_idle`/`system_audio_heard`) but the **system-audio trigger is shelved (EXPERIMENTAL, `auto_capture_enabled`=false)**: macOS reports the output device as perpetually "running" from inside the app once a tap is opened, so the tap-free sensor false-triggers in-app (proven live). Stop-detection (captured-audio RMS) + probation (discard false starts, no junk rows) **work**; the next step is a **mic-VAD trigger** ("start when you speak"). 98 Rust tests green.
- **Next (for Gianni / incoming dev):** read `docs/HANDOFF-GIANNI.md`. Then: the **"Enable diarization" download button**; the **mic-VAD auto-trigger** pivot; the AI title/summary (after the §12 provider call). See `docs/HANDOFF.md` §11.
